...
The parameters of the nominal signal model are listed in Table~\ref{tab:Fit_Par_Overview_Sig}.
\begin{table}[b]
\begin{center}
 \renewcommand{\arraystretch}{1.3}
...
$N_{Z}^{C}$ & floating & pred. from data & 0 & $N_{SF}^{C}(\text{on-Z})$ \\
$N_{Z}^{F}$ & floating & pred. from data & 0 & $N_{SF}^{F}(\text{on-Z})$ \\ \hline
